...
<html>
	...
	<body>
					...
					<tr>
						...
						<td>
							<c:forEach begin="1" end="${bvo.bindent}" step="1">
								...
								<span class="material-symbols-outlined">hotel_class</span></c:if></a>
						</td>
						<td>${bvo.id }</td>
						
						<td>
							<fmt:formatDate value="${bvo.bdate }" pattern="yyyy-MM-dd"/><!--mm으로 넣으면 분으로 나와  -->
		...
		<!-- 하단넘버링 부분 -->
			...
			<!-- 다음페이지 이동 -->
			<c:if test="${map.page==maxpage}"><span>&#62;</span></c:if>
