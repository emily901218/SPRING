...
<html>
	...
	<body>
		<!-- top 시작부분 -->
	 	...
	 	<script>
	 		function termsBtn(){
				alert("agree 갯수: "+$("input:radio[value='agree']:checked").length);
				//모두 체크가 안되어있으면 모두 체크를 물어볼 수 있도록 하도, 예스하면 모두 체크가 되도록 해보세요.
				if($("input:radio[value='agree']:checked").length==3){
					agree.submit();
				}else{
					if(confirm("3개의 항목을 모두 동의하셔야 가입이 가능합니다.모두 동의체크할까요?")){
						//모두체크
		...
		<section>
			<form action="join02_info_input.do" name="agree" method="get" action="join02_info_input.html">
				...
				<div id="terms_button">
					<input type="button" onclick="cancleBtn()" value="취소하기" />
