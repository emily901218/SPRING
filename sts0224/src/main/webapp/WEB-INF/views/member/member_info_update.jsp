...
<html>
	<head>
			...
			<style>
				...
				.offIdCheck{color:blue; font-weight:600;}
			...
			<script>
				...
				//회원정보수정버튼
				function updateBtn(){
					//alert("hobby 체크개수 : "+$("input:checkbox[name=hobby]:checked").length);	
					if($("input:checkbox[name=hobby]:checked").length==0){
						alert("취미를 1개 이상 체크하셔야 회원가입이 가능합니다.");
						return;
					}
					//*****입력패턴 체크*****
...
// 						alert("패스워드는 3자리 이상 10자리 이하 영문, 숫자,특수문자 1개 이상 가능합니다.");
// 						return ;
// 					}
					agree.submit();
				}//joinBtn
					...
					<dl id="join_interests_dl">
						...
						<dd>
							<ul>
								...
								<li>
									<input type="checkbox" name="hobby" id="health" value="health" <c:if test="${fn:contains(mvo.hobby,'health') }">checked</c:if> />
