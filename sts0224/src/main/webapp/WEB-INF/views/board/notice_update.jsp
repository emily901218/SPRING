...
<head>
  ...
  <script>
  	function cancelBtn(){
  		alert("test");
  		if(confirm("게시글 수정을 취소하시겠습니까?")) location.href ="notice_list.do?page="+${page};
  	}
  	function updateBtn(){
  		if($("#btitle").val()==""){
  			alert("게시글 제목을 입력하셔야 등록이 가능합니다.");
  			$("#btitle").focus();
  			return;
  		}
  ...
  <section>
    ...
    <input type="hidden" name="oldfile" value="${bvo.bfile }">
      ...
      <div class="button-wrapper">
        <button type="button" onclick="updateBtn()" class="write">수정</button>
